...
namespace TRSEvaluation
{
    internal class Program
    {
        ...
        {
            var uniquesInValues = values.Distinct();
            var itemAndOccurences = new List<KeyValuePair<string, int>>();

            foreach (string i in uniquesInValues)
            {
                itemAndOccurences.Add(new KeyValuePair<string, int>(i, values.Count(c => c == i)));

            }
            return itemAndOccurences;
        }
